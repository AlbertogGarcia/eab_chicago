...
ATT_gain <- round(df[1,2], digits = 5)
ATT_loss <- round(df[2,2], digits = 5)
se_gain <- paste0("(", round(df[1,3], digits = 5), ")")
se_loss <- paste0("(", round(df[2,3], digits = 5), ")")

kab <- data.frame("loss" = c(ATT_loss, se_loss),
                  "gain" = c(ATT_gain, se_gain))
kable(kab, booktabs = T, col.names = c("Tree cover loss", "Tree cover gain"))
```
...
So far, I have shown that confirmed EAB infestation leads to loss of tree cover and lower test scores at exposed schools. In this section, I will seek to estimate the test score impacts of tree cover. 


## First stage

\begin{align}
canopy_{st} = \beta_0 + \beta_1 \times infestation_{st} + \gamma_t + \lambda_s + \epsilon_{it} 
\end{align}
